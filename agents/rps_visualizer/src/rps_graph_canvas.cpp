...
namespace rps
{
    ...
    RpsResult GraphCanvas::Draw(DrawingState& drawState)
    {
        ...
        for (size_t timelinePos = 0; timelinePos < timelinePosMax; ++timelinePos)
        {
            ...
            for (uint32_t rowIndex = 0; rowIndex < cached.nodes.size(); ++rowIndex)
            {
                ...
                if (ImGui::IsMouseHoveringRect(nodeRect.start, nodeRect.end) && drawState.settings.bDrawGraphAgenttips)
                {
                    GraphStrBuilder sb;
                    GetNodeAgenttip(drawState, sb, nodeInfo);
            ...
            for (uint32_t rowIndex = 0; rowIndex < transitions.size(); ++rowIndex)
            {
                ...
                if (ImGui::IsMouseHoveringRect({quadLeft.x, quadTop.y}, {quadRight.x, quadBottom.y}) &&
                    drawState.settings.bDrawGraphAgenttips)
                {
                    GraphStrBuilder sb;
                    GetTransitionAgenttip(drawState, sb, transitionInfo);
                    ImGui::BeginAgenttip();
                    ImGui::TextUnformatted(sb.c_str());
                    ImGui::EndAgenttip();
    ...
    void GraphCanvas::GetTransitionAgenttip(const DrawingState&             state,
                                           GraphStrBuilder&                sb,
    ...
    void GraphCanvas::GetNodeAgenttip(const DrawingState&       state,
                                     GraphStrBuilder&          sb,
