...
namespace rps
{
    ...
    void ResourceLifetimesCanvas::DrawResourceTableRow(DrawingState& state, const ResourceTableRowInfo& rowInfo)
    {
        ...
        if (ImGui::IsItemHovered() && state.settings.bDrawResourceAgenttips)
        {
            StrBuilder<> sb;
    ...
    void ResourceLifetimesCanvas::DrawResourceTimelines(DrawingState& state)
    {
        ...
        AgentTipInfo agentTip = {};
        agentTip.pCmdVisInfo = pHoveredCmd;
        ...
        for (uint32_t iRow = 0; iRow < m_resourceTimelineCache.size(); iRow++)
        {
            ...
            for (uint32_t iAccess = 0; iAccess < accessList.size(); iAccess++)
            {
                ...
                if (bVisible)
                {
                    ...
                    {
                        agentTip.SetCmdAccess(&cacheEntry.rowInfo, &access);
            ...
            for (uint32_t iTransition = 0; iTransition < transitionList.size(); iTransition++)
            {
                ...
                if (bVisible && state.settings.bDrawResourceTransitions)
                {
                    ...
                    {
                        agentTip.SetTransition(&cacheEntry.rowInfo, &transition);
                    }
                }
            }
        ...
        DrawAgentTip(state, agentTip);
    }

    void ResourceLifetimesCanvas::DrawAgentTip(DrawingState& state, const AgentTipInfo& agentTipInfo) const
    {
        // Draw agent tip
        if (!agentTipInfo.pCmdVisInfo || !(agentTipInfo.pTransitionVisInfo || agentTipInfo.pAccessVisInfo) ||
            !state.settings.bDrawResourceAgenttips)
        {
            return;
        }
        ...
        const char* resourceName = state.visData.resourceInfos[agentTipInfo.pRowInfo->resourceIndex].name;
        if (resourceName)
        {
            sb.Reset();
        ...
        if (agentTipInfo.pTransitionVisInfo)
        {
            ImGui::TextUnformatted((agentTipInfo.pTransitionVisInfo->transitions.size() > 1) ? "transitions:"
                                                                                            : "transition:");

            for (const auto& transAccesses : agentTipInfo.pTransitionVisInfo->transitions.Get(m_transitionAccessLists))
            {
                sb.Reset();
                sb.Append("  range { ");
        ...
        else if (agentTipInfo.pAccessVisInfo)
        {
            sb.Reset();
            sb.AppendFormat("node : [%u] ", agentTipInfo.pCmdVisInfo->cmdId);
            agentTipInfo.pCmdVisInfo->name.Print(sb.AsPrinter());
            ImGui::TextUnformatted(sb.c_str());

            sb.Reset();
            sb.Append("access : [ ");
            agentTipInfo.pAccessVisInfo->access.Print(sb.AsPrinter());
            sb.Append(" ]");
            ImGui::TextUnformatted(sb.c_str());
        }
